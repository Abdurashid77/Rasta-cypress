describe('example to-do app', () => {
    ...
    it('Create order with card', () => {
        ...
        cy.wait(300)
        cy.get('.CheckoutPayment_checkbox__nYwDg').click({force: true})
        cy.wait(300)
        cy.get('.CheckoutPriceInfo_priceInfo__HIlqr > .Button_greenButton__UlmDG').click({force: true})
        cy.wait(1000)
        cy.get('.OrderSuccessDialog_dialog__Md6Je > .Button_greenButton__UlmDG').click({force: true})
        cy.wait(300)
        cy.get('.Header_profile__OULED').click({force: true})
        cy.wait(300)
        cy.get('[href="/user/orders"]').click({force: true})

        //End-search-restaurant-and-create-order
    })
    })